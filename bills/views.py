...
def analyze_bill_automatically(bill):
    ...
    PRIORITY 1: Check if PAID (highest priority - paid bills always Low Risk)
    PRIORITY 2: Overdue detection
    PRIORITY 3: Amount/Units thresholds
    PRIORITY 4: AI-based analysis for electricity
    """
    
    # ========== PRIORITY 1: CHECK IF PAID (HIGHEST PRIORITY) ==========
    if bill.is_paid:
        # Bill is PAID - always Low Risk, no anomaly
        bill.late_payment_risk = 'Low Risk'
        bill.is_anomaly = False
        bill.save()
        return False, 'Low Risk'
    
    # ========== PRIORITY 2: CHECK IF OVERDUE ==========
    ...
    # ========== PRIORITY 3 & 4: ELECTRICITY BILLS (AI-BASED) ==========
...
def toggle_bill_payment(request, bill_id):
    """Toggle bill payment status - Automatically updates risk status"""
    if request.method == 'POST':
        ...
        # ✅ NEW: Automatically update risk when payment status changes
        if bill.is_paid:
            # Bill is now PAID - clear high risk and anomaly
            bill.late_payment_risk = 'Low Risk'
            bill.is_anomaly = False
            bill.save()
            messages.success(request, f'✅ Bill marked as PAID! Risk status cleared.')
        else:
            # Bill is now UNPAID - re-analyze to determine risk
            bill.save()  # Save first to ensure is_paid = False
            is_anomaly, payment_risk = analyze_bill_automatically(bill)
            
            if payment_risk == 'High Risk':
                messages.warning(request, f'⚠️ Bill marked as UNPAID! High risk status restored.')
            else:
                messages.success(request, f'✅ Bill marked as unpaid.')
